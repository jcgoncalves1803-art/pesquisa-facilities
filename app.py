...
com tab_avaliar:
    ...
    outro:
        ...
        com st.form("form_avaliacao", clear_on_submit=True):
            ...
            transporte_nota = st.radio("Qual sua satisfação com o serviço de transporte?", options=[1, 2, 3, 4, 5], format_func=lambda x: ESCALA[x], horizontal=True, key="transporte_nota")
            transporte_comentario = st.text_input("Compartilhe seus comentários sobre o transporte:", key="transporte_com")
            st.divider()

            st.markdown("### 2. Portaria")
            portaria_nota = st.radio("Qual sua satisfação com o serviço de portaria?", options=[1, 2, 3, 4, 5], format_func=lambda x: ESCALA[x], horizontal=True, key="portaria_nota")
            portaria_comentario = st.text_input("Compartilhe sua opinião sobre a portaria:", key="portaria_com")
            st.divider()

            st.markdown("### 3. Vigilância")
            vigilancia_nota = st.radio("Qual sua satisfação com o serviço de vigilância?", options=[1, 2, 3, 4, 5], format_func=lambda x: ESCALA[x], horizontal=True, key="vigilancia_nota")
            vigilancia_comentario = st.text_input("Compartilhe sua opinião sobre a vigilância:", key="vigilancia_com")
            st.divider()

            st.markdown("### 4. Limpeza")
            limpeza_nota = st.radio("Qual sua satisfação com o serviço de limpeza?", options=[1, 2, 3, 4, 5], format_func=lambda x: ESCALA[x], horizontal=True, key="limpeza_nota")
            limpeza_comentario = st.text_input("Compartilhe sua opinião sobre a limpeza:", key="limpeza_com")
            st.divider()

            st.markdown("### 5. Jardinagem")
            jardinagem_nota = st.radio("Qual sua satisfação com o serviço de jardinagem?", options=[1, 2, 3, 4, 5], format_func=lambda x: ESCALA[x], horizontal=True, key="jardinagem_nota")
            jardinagem_comentario = st.text_input("Compartilhe sua opinião sobre a jardinagem:", key="jardinagem_com")
            st.divider()

            st.markdown("### 6. Refeição")
            refeicao_nota = st.radio("Qual sua satisfação com o serviço de refeicao?", options=[1, 2, 3, 4, 5], format_func=lambda x: ESCALA[x], horizontal=True, key="refeicao_nota")
            refeicao_comentario = st.text_input("Compartilhe sua opinião sobre as refeições:", key="refeicao_com")
            st.divider()

            st.markdown("### 7. Satisfação Geral (NPS)")
            nps = st.slider("Em uma escala de 0 a 10, qual é sua satisfação geral com os serviços de Facilities?", min_value=0, max_value=10, value=5, key="nps")
            st.divider()

            st.markdown("### 8. Qual serviço precisa de mais atenção?")
            servico_atencao = st.multiselect("Selecione um ou mais:", options=SERVICOS, key="servico_atencao")
            st.divider()

            st.markdown("### 9. Percebeu melhoria nos últimos 6 meses?")
            observar_melhoria = st.radio("Você está vendo alguma melhoria nos serviços de Facilities?", options=["Sim", "Nao", "Não tenho certeza"], horizontal=True, key="melhoria")
            st.divider()

            st.markdown("### 10. Sugestões")
            sugestões = st.text_area("Deixe suas ideias ou sugestões para aprimorar os serviços:", max_chars=1000, key="sugestões")
            st.divider()
            ...
            com col2:
                ...
                outro:
                    município = st.selectbox("Município:", [""])
            st.divider()

            submitted = st.form_submit_button("Enviar Avaliação", use_container_width=True, type="primary")
...
com tab_resultados:
    ...
    outro:
        notas_cols = ["nota_transporte", "nota_portaria", "nota_vigilancia", "nota_limpeza", "nota_jardinagem", "nota_refeicao"]
        ...
        charts.ranking_barras([r[0] para r em ranking_data], [r[1] para r em ranking_data])
        se "nps" em df.columns:
        ...
        if "percebeu_melhoria" em df.columns:
            ...
            config_melhoria = {
                ...
                "series": [{"name": "Respostas", "data": [{"name": str(nome), "y": int(qtd)} for nome, qtd in melhoria_count.items()]}],
            }

...
class HighchartsRenderer:
    ...
    def gauge_nps(valor):
        config = {
            "chart": {"type": "gauge", "plotBackgroundColor": None, "plotBorderWidth": 0, "plotShadow": False, "height": "80%"},
            "title": {"text": "NPS - Satisfacao Geral"},
            "pane": {"startAngle": -90, "endAngle": 89.9, "background": None, "center": ["50%", "75%"], "size": "110%"},
            "yAxis": {"min": 0, "max": 10, "tickPixelInterval": 72, "tickPosition": "inside", "tickLength": 20, "tickWidth": 2, "labels": {"distance": 20, "style": {"fontSize": "14px"}}, "plotBands": [{"from": 0, "to": 6, "color": "#DF5353", "thickness": 20}, {"from": 6, "to": 8, "color": "#DDDF0D", "thickness": 20}, {"from": 8, "to": 10, "color": "#55BF3B", "thickness": 20}]},
            "series": [{"name": "NPS", "data": [round(valor, 1)], "dataLabels": {"format": "{y}/10", "borderWidth": 0, "style": {"fontSize": "24px"}}, "dial": {"radius": "80%", "backgroundColor": "gray"}, "pivot": {"backgroundColor": "gray", "radius": 6}}]
        }
    ...
    def column_drilldown(categorias, valores):
        ...
        config = {
            ...
            "plotOptions": {"series": {"borderWidth": 0, "dataLabels": {"enabled": True, "format": "{point.y:.1f}"}, "colorByPoint": True}},
    ...
    def stacked_bar(categorias, dados):
        series = []
        cores = {5: "#27ae60", 4: "#2ecc71", 3: "#f39c12", 2: "#e67e22", 1: "#e74c3c"}
        nomes = {5: "Muito satisfeito (5)", 4: "Satisfeito (4)", 3: "Neutro (3)", 2: "Insatisfeito (2)", 1: "Muito insatisfeito (1)"}
        for nota in [5, 4, 3, 2, 1]:
            if nota in dados:
                series.append({"name": nomes[nota], "data": dados[nota], "color": cores[nota]})
        config = {
            "chart": {"type": "bar"},
    ...
    def spider_web(categorias, medias):
        config = {
            ...
            "pane": {"size": "80%"},
            "xAxis": {"categories": categorias, "tickmarkPlacement": "on", "lineWidth": 0},
    ...
    def wordcloud(palavras):
        ...
        config = {
            "series": [{"type": "wordcloud", "data": data, "name": "Frequencia"}],
    ...
    def pie_detalhado(dados):
        config = {
            ...
            "plotOptions": {"pie": {"allowPointSelect": True, "cursor": "pointer", "dataLabels": {"enabled": True, "format": "<b>{point.name}</b>: {point.percentage:.1f}%"}, "showInLegend": True}},
    ...
    def ranking_barras(fornecedores, medias):
        ...
        config = {
            ...
            "series": [{"name": "Media", "data": [{"y": m, "color": colors[i % 6]} for i, m in enumerate(medias)], "showInLegend": False}],
    ...
    def bar_race(dados_servicos):
        ...
        data = [{"name": cat, "y": val, "color": cores[i % 6]} for i, (cat, val) in enumerate(zip(categorias, valores))]
        config = {
